...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for i in range(len(livingGhostPositionDistributions)):
            position = None
            prob = 0
            for pos in livingGhostPositionDistributions[i]:
                pr = livingGhostPositionDistributions[i][pos]
                if pr > prob:
                    prob, position = pr, pos
        ...
        for action in legal:
            for ghost in ghostPos:
                ...
                dist = self.distancer.getDistance(pos, ghost)
                if dist < minDist:
                    minDist, bestAction = dist, action
