...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = [self.legalPositions[i%n] for i in range(self.numParticles)]
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            w = self.getObservationProb(observation, pacman, particle, jail)
            weights[particle] += w
    ...
    def elapseTime(self, gameState):
        ...
        for particle in self.particles:
            ...
            elapsedParticles.append(newParticle)
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        uniform prior.
        """
        cartesianProd = itertools.product(self.legalPositions, repeat=self.numGhosts)
        positionTuples = [p for p in cartesianProd]
        random.shuffle(positionTuples)
        n = len(positionTuples)
        self.particles = [positionTuples[i%n] for i in range(self.numParticles)]
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                w *= self.getObservationProb(observation[i], pacman, particle[i], self.getJailPosition(i))
            weights[particle] += w
