...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        bestAction = random.choice(possibleActions)
        return bestAction
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        return featureValues * self.weights
    ...
    def update(self, state, action, nextState, reward):
        ...
        maxQ = 0
        if len(actions) > 0:
          maxQ = max(self.getQValue(nextState, a1) for a1 in actions)
        diff = reward + self.discount * maxQ - self.getQValue(state, action)
        featureValues = self.featExtractor.getFeatures(state, action)
        for f in featureValues:
          fVal = featureValues[f]
          self.weights[f] += self.alpha * diff * fVal
